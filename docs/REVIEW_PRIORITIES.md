...
Implementation status: the first hardening pass implements the top three items: `lth verify` with `done` blocking, atomic JSON writes plus file-based run locks, and safer Codex cwd/sandbox defaults.
...
| P1 | JSON schemas are not connected to validation. | `schemas/*.schema.json` exist, but `loadAndValidate` only performs a small hand-written subset and does not check `schemaVersion === 1`. | Add AJV or a small local subset validator, and give explicit migration guidance for unknown schema versions. |
